...
### 사진·영상 (2026-07-04 투입)
- **의료진 실사 프로필 5인** — 고경우·장유진·박진아·변진수·강세원 (명찰 판독으로 본인 매칭, 3:4 세로 크롭 webp)
- **대표원장 인터뷰 영상** (`/static/media/interview.mp4`, 1080p·8.2MB·한글자막) — 홈 + 병원소개 페이지 임베드, VideoObject 구조화데이터
- **홈**: 히어로(대기실 라운지), 소개(원장 진료컷), 인터뷰 섹션, 공간 갤러리 8컷(소아센터·키즈존·CT실·독립진료실 등)
- **병원소개**: 미션 월, 인터뷰 영상, 진료 현장 2컷
- **의료진**: 5인 단체사진 + 개별 프로필
- **진료 상세**: 진료별 현장 사진 (임플란트/교정/소아/보철/잇몸/일반 각 1컷)
- **오시는길**: 주차장 사진 2컷 + "당일 진료 3시간 무료주차" 안내 반영
...
## 10. 납품일 최종 검수 결과 (2026-07-04)

| # | 점검 항목 | 결과 |
|---|-----------|------|
| 1 | TypeScript 타입체크 + 빌드 | ✅ 에러 0 / 920 kB 빌드 성공 |
| 2 | 사이트맵 전체 URL (583개) | ✅ 전수 200 (버그 1건 발견·수정: 비공개 공지가 사이트맵에 노출되던 문제 → published=1 필터 추가) |
| 3 | SEO 정밀검증 18페이지 | ✅ title/description(50자+)/canonical/H1 단일/img alt/U+FFFD 전부 통과 |
| 4 | JSON-LD 구조화데이터 | ✅ 전 페이지 파싱 성공 (프로덕션 재확인 포함) |
| 5 | 내부 링크 크롤링 (60페이지, 158링크) | ✅ 깨진 링크 0 (glossary 동적 JS 템플릿 오탐 1건 제외 — 실제 링크는 정상) |
| 6 | 의료광고법 금칙어 스캔 | ✅ 최상급/보장성 표현 0건 |
| 7 | 예약 폼 E2E (POST → D1 저장) | ✅ 접수·저장·관리자 상태 확인 정상 |
| 8 | 브라우저 콘솔 에러 (홈/용어사전/예약) | ✅ 0건 |
| 9 | 보안 헤더 (프로덕션) | ✅ HSTS·X-Frame·nosniff·Referrer-Policy 적용 |
| 10 | 프로덕션 배포 (isoldc.kr) | ✅ 재배포 완료, 주요 라우트 전부 200 |
