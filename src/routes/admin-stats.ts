...
import { adminShell, requireAdmin } from './admin';
...
// ────────────────────────────────────────────────────────────
export const adminStats = new Hono<{ Bindings: Bindings }>();
...
adminStats.get('/stats', async (c) => {
  ...
  <h1>검색·방문 통계</h1>
  <p class="sub">구글 검색 노출과 사이트 방문 현황을 한눈에 확인합니다.</p>
  ${statsBody(d)}`));
});
...
export const localStats = new Hono<{ Bindings: Bindings }>();

localStats.get('/local-stats', async (c) => {
  const key = c.req.query('key');
  if (key !== STATS_KEY && key !== STATS_MASTER_KEY) return c.text('Not Found', 404);
  const db = c.env.DB;
